...
author: Kevin Kelche
title: Golang Scope and Lifetime of Variables
description: Learn about the scope and lifetime of variables in Go programming language. Understand memory management and best practices for effective variable usage
date: 08 March 2023
image: /images/golang-scope.svg
imageDescription:
keywords:
  -
tags:
...
In the above example, the `someFunc` function initializes variable `i` which can be accessed by the `Printf` function withing the same function. If we were to access the variable `i` outside the function it would result in a compile error.
...
By running this program you'll see that we are able to access the variables from the package `store` from the `main` package. If `SomeVar` or `DataIsBeautiful` were declared in lowercase it would have resulted in compile time errors.

## Lifetimes

As mentioned earlier a Lifetime is the time a variable exists in the memory. In Golang, the lifetime of a variable is determined by the scope of the variable. The lifetime of a variable is the same as the scope of the variable, meaning that if a variable is declared in a block, it will be destroyed when the block is exited. This is because a stack frame is created for each block and the variables are stored in the stack frame. When the block is exited, the stack frame is destroyed and the variables are destroyed with it.
...
When dealing with package-scoped variables the lifetime is the same as the lifetime of the program. This is because the variables are stored in the heap and are not destroyed when the function is exited. This is the same as when a function returns a pointer to a variable within that function.

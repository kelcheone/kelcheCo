...
Golang maps can also be refered to as hash tables (maps) or dictionaries. It is one of the most important data structures in programming as it allows us to store key-value pairs. We are going to explore the map it's usage in Golang and how it differs from other data structures.

## Creating a Map

Map can be created in two ways in Golang. The first way is by using the `make` function and the second way is by using map literals. The `make` function is used to create a map with a specific size. The map literals are used to create a map with initial values.

...
description: The Golang scheduler is responsible for scheduling goroutines to kernel threads. In this article, we take a look at how the scheduler works.
date: 26 January 2023
image: /images/golang-scheduler.svg
...
What makes Go, a great language to work with is the concept of concurrency. Concurrency in its most basic definition is the ability to run multiple tasks at the same time. This is a very powerful concept, but it comes with a lot of challenges. Thank god for the Go team, they have made it very easy to work with concurrency in Go. In this article, we take a look under the hood of the Go runtime and see how the scheduler works.

### Some Prehumble <SectionLink id="some-prehumble" />
...
## The Scheduler <SectionLink id="the-scheduler" />
...
## The M:N Scheduler <SectionLink id="the-mn-scheduler" />
...
### States of a Goroutine <SectionLink id="states-of-a-goroutine" />
...
## How Does the Scheduler keep track of Goroutines? <SectionLink id="how-does-the-scheduler-keep-track-of-goroutines" />
...
## Fairness <SectionLink id="fairness" />
...
## How Fairness was Achieved in the Go Scheduler <SectionLink id="how-fairness-was-achieved-in-the-go-scheduler" />

### Preemption <SectionLink id="preemption" />
...
### Local Run Queues <SectionLink id="local-run-queues" />
...
### M:P:N Threading Model <SectionLink id="m-p-n-threading-model" />
...
### Work Stealing <SectionLink id="work-stealing" />
...
The tick was set to 61 because it is not too small to cause checking of the global run queue too often and not too big to cause starvation of goroutines in the global run queue. Another reason is that 61 is a prime number and prime numbers are great for hashing. If 64 is used, it will cause a lot of collisions. These collisions would interfere with the go runtime patterns and result in unfairness.
...
As you can see in the above diagram, processor 2 will steal only 2 goroutines from the global run queue. This is because to get a fair chance, each processor should only steal the equivalent of the number of goroutines available by the number of processors. In this case, we have 3 processors, and 6 goroutines in the global run queue. This means that each processor should steal 2 goroutines.

### Network Poller <SectionLink id="network-poller" />
...
### Cases where the global run queue, local run queue, and network poller are empty <SectionLink id="cases-where-the-global-run-queue-local-run-queue-and-network-poller-are-empty" />

There are cases where the global run queue and local run queue are empty and the network poller is also empty or has blocked goroutines. In this case, the processor will then have to steal work from another processor. The processor randomly picks a processor to steal work from (including itself). When it finds a processor with runnable goroutines, it will steal half of the goroutines from the other processor and add them to its run queue.
...
### Fairness Hierarchy <SectionLink id="fairness-hierarchy" />
...
## Handoff <SectionLink id="handoff" />
...
### What happens when you return from a handed-off syscall? <SectionLink id="what-happens-when-you-return-from-a-handed-off-syscall" />
...
## Rutime APIs <SectionLink id="rutime-apis" />

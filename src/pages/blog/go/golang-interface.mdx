...
- [How to Declare an Interface?](#how-to-declare-an-interface)
- [How to Implement an Interface?](#how-to-implement-an-interface)
- [How to Use an Interface as a Type?](#how-to-use-an-interface-as-a-type)
- [Empty Interface](#empty-interface)
- [Type Assertion in Interfaces](#type-assertion-in-interfaces)
- [Type Switch in Interfaces](#type-switch-in-interfaces)
- [Interface in variadic functions](#interface-in-variadic-functions)
- [Multiple Interfaces in a struct](#multiple-interfaces-in-a-struct)
...
<Nugget>
  <p>
    Interfaces provide object-oriented like approach in Go ( Polymorphism ),
...
There is alot going on in the above example. Let's break it down.

First, we have declared two types: `Book` and `Drink`. Both the types have a `price` field. The `Book` type has two more fields: `title` and `author`. The `Drink` type has a `name` field. Both the types have a `printIt` method. The `printIt` method takes no parameters and returns nothing.
...
If we addedd a new type to the `Printer` interface, we would have to implement the `printIt` method on the new type as well. Also by introducing a foreign type to the `info` slice, we would have to implement the `printIt` method on the foreign type as well or else we would get a compile time error.

<Nugget>
  <p>
    <strong>Interfaces are implemented implicitly</strong>
  </p>
  <p>
    There is no explicit declaration of intent, no "implements" keyword. If a
    type provides all the methods required by an interface, it is said to
    implement the interface.
  </p>
</Nugget>

## How to Use an Interface as a Type?
...
This is important in cases where we don't know the type of the value that we are working with.
...
## Type Switch in Interfaces

Type switch is a way to check the type of a value. The syntax for type switch is as follows:
...
In the above example we declare function `do` which takes an empty interface as a parameter. The function `do` uses a type switch to check the type of the value passed to it. The function `do` prints the value of the value passed to it. We have called the `do` function with three different types of values. The `do` function is called with a value of type `int`, `string` and `bool`. The `do` function prints the value of the value passed to it.

## Interfaces in variadic functions
...
You can complement variadic functions with interfacess and switch statements to create a powerful tool for handling different types of data.

## Multiple Interfaces in a struct
...
Golang does not allow multiple inheritance. However, it does allow embedding of interfaces. This is by merging the methods of two or more interfaces into a single interface. Let's take a look at an example:

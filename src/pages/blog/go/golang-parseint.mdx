...
Some other methods related to `strconv.ParseInt` are `strconv.ParseUint`, `strconv.ParseFloat`, and `strconv.ParseBool`. These methods convert a string to an unsigned integer, float, and boolean, respectively. Learn also about [`strconv.Atoi()`](/blog/golang-atoi/).

...
Thre are two ways to copy a slice in Golang. You can use the built-in `copy` function or use the `append` function.
...
If the destination slice is smaller than the source slice, the copy function will only copy the number of elements that fit in the destination slice.
...
<Nugget>
  <p>
    ...
    value of the element type. In the example above, the zero value of an
    integer is 0.
  </p>
</Nugget>

The copy function returns the number of elements copied. This value can be used to check if the copy was successful.
...
To make sure all the elements from the source you could get the length of the source slice and use it to create the destination slice.
